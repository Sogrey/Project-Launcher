...
- **脚本过滤** — 自动忽略 `package.json` 里以 `//`、`#`、`@` 等非法字符开头的“注释脚本”
- **一键启停 / install** — 支持 npm / pnpm / yarn（按项目自动检测：`packageManager` 字段 → 锁文件 → 默认 npm）；可停单脚本、停项目全部脚本、一键全停
...
- **脚本识别**：只列出可运行脚本名（字母/数字/`_` 开头，且符合 `a-zA-Z0-9_:-`）；以 `//`、`#`、`@`、`/*` 等开头的伪注释 key 会被过滤
- **脚本同步**：应用启动时在后台重新读取各项目的 `package.json`（不阻塞开抽屉），新增/删除的脚本会自动更新

### 看板布局

| 区域 | 说明 |
|------|------|
| **项目列表** | 工作区内全部项目；可拖拽排序；点击打开管理抽屉（可打开目录 / 用已检测 IDE 打开） |
| **运行中** | 每个运行中的脚本一条；点击后在右上显示对应日志；有端口时显示可点击链接 |
| **日志输出** | 当前选中运行中或异常任务的输出；可点「取消」清除焦点 |
| **异常** | 非正常退出记录；点击在右上查看该任务日志（不打开抽屉）；可清除 |
...
2. 抽屉内可「打开目录」（资源管理器）或选择已安装的 IDE（VS Code / Cursor / Trae CN / CodeBuddy (CN|Intl) 等）。Windows 下优先读卸载注册表定位自定义安装路径，其次 PATH，最后才是常见默认目录
3. 无日志焦点时，新启动的任务会自动出现在右侧；已有焦点时保持当前任务（避免批量启动乱跳）
4. 也可在中间列手动点选要看的任务
5. 停止方式：运行中卡片 ■、抽屉内按脚本停止 / 停止全部、项目卡片 ■■、顶栏「一键全停」
